...
describe Hand do
  ...
  describe "#intialize" do
    it "should set cards in the hand" do
      hand.cards.should == pair
    end
    
    it "should not allow two cards of the same rank and suit" do
      expect { Hand.new(bad_hand)}.to raise_error
    end
  end
  
  describe "#hand" do
    it "should determine what type of hand it is" do
      Hand::HANDS[hand.hand].should == "pair"
      Hand::HANDS[Hand.new(flush).hand].should == "flush"
    end
  end
  
  describe "#buckets" do
    it "should sort the cards into buckets based on rank" do
      buckets = hand.send(:buckets)
      buckets.count.should == 13
      buckets.first.count.should == 0
      buckets[1].count.should == 2
      buckets.map {|b| b.count }.inject(&:+).should == hand.cards.count
    end
    
    it "should include the original cards" do
      buckets = hand.send(:buckets)
      cards = buckets.inject(&:+)
      cards.each do |card|
        pair.include?(card).should be_true
      end
    end
    
    it "should return an array of arrays with Cards" do
      buckets = hand.send(:buckets)
      buckets.count.should == 13
      buckets.first.class.should == Array
      buckets[1].first.class.should == Card
    end
  end
  ...
  describe "#kickers" do
    describe "when you have a pair" do
      it "should have three kickers" do
        hand.kickers.count.should == 3
      end
      
      it "should reverse sort them" do
        hand.kickers.should == hand.kickers.sort.reverse
      end
    end
    
    describe "when you have a flush" do
      it "should have five kickers" do
        Hand.new(flush).kickers.count.should == 5
      end
      
      it "should reverse sort them" do
        Hand.new(flush).kickers.should == Hand.new(flush).kickers.sort.reverse
      end
    end
    
    describe "when you have a full house" do
      it "should have zero kickers" do
        Hand.new(full_house).kickers.empty?.should be_true
      end
    end
    
    describe "when you have a straight" do
      it "should have five kickers" do
        Hand.new(straight).kickers.count.should == 5
      end
      
      it "should reverse sort them" do
        Hand.new(straight).kickers.should == Hand.new(straight).kickers.sort.reverse
      end
    end
    
    describe "when you have a set" do
      it "should have two kickers" do
        Hand.new(set).kickers.count.should == 2
      end
      
      it "should reverse sort them" do
        Hand.new(set).kickers.should == Hand.new(set).kickers.sort.reverse
      end
    end
  end
  
  describe "#compare_kickers" do
    describe "when they are unequal" do
      it "should pick the high card" do
        Hand.new(ten_high).compare_kickers(Hand.new(seven_high).kickers).should == 1
      end
    end
    
    describe "when they are equal" do
      it "should return 0" do
        hand.compare_kickers(Hand.new(pair).kickers).should == 0
      end
    end
  end
